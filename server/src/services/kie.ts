...
import { applyStressMarks } from './stressMarker.ts'
...
interface GenerateParams {
    ...
    language?: string
    productUrl?: string
    isContinuation?: boolean // Flag to indicate if this is a continuation scene
}
...
interface TaskStatusResponse {
    ...
    data: {
        ...
        successFlag: number // 1 = success, 0 = processing, other = failed
...
function buildUGCDirectorInstructions(productUrl?: string, isContinuation?: boolean): string {
    ...
    if (productUrl) {
        ...
        // CRITICAL: Product must be visible in final frame for scene continuity
    ...
    // Scene ending - complete word and still pose
...
export const kieService = {
    ...
    async generateVideo({ prompt, imageUrl, aspectRatio = '16:9', watermark = 'UGC Creator', callBackUrl, language, productUrl, isContinuation = false }: GenerateParams): Promise<string> {
        ...
        try {
            ...
            // Build final prompt: Script with stress marks + UGC Director Instructions
            // Apply stress marks to Bulgarian text for proper TTS pronunciation
            const stressedScript = applyStressMarks(prompt)
            let finalPrompt = stressedScript

            // Add UGC Director instructions for avatar behavior (without stress marks)
            ...
            const payload: any = {
                prompt: finalPrompt,
                imageUrls,
                model: 'veo3_fast', // Using veo3_fast as requested
                callBackUrl,
                aspectRatio,
                ...
                watermark: watermark || 'UGC Creator' // Add watermark as per docs
            }

            // No watermark for any scene
            ...
            console.log('[Kie.ai] Generation response status:', response.status)
            console.log('[Kie.ai] Generation response data code:', response.data?.code)

            if (response.data.code === 200 && response.data.data?.taskId) {
                console.log('[Kie.ai] Task ID received:', response.data.data.taskId)
