...
if (!FAL_KEY) {
    console.warn('Warning: Missing FAL_KEY. Frame extraction will not work.')
}
...
export const falService = {
    ...
    async combineVideos(sceneUrls: string[]): Promise<{ requestId: string }> {
        // User spec: 8 seconds per scene
        console.log('[Fal Service] Combining videos:', sceneUrls)
        ...
        console.log('[Fal Service] Keyframes payload:', JSON.stringify(keyframes, null, 2))
        ...
        // User says response is array
        const data = Array.isArray(response.data) ? response.data[0] : response.data
        console.log('[Fal Service] Combine response:', JSON.stringify(data, null, 2))
