...
export const videoApi = {
    ...
    // Start next scene generation (frame extraction)
    ...
    // Generate scene with extracted frame
    ...
    // Check status of a specific scene
    getSceneStatus: async (generationId: string, sceneTaskId: string) => {
        // Need parent taskId for lookup? The server route is /scene-status/:taskId/:sceneTaskId
        // But here we accept generationId. 
        // We should probably rely on the fact that sceneTaskId is enough OR we need taskId.
        // Wait, server route is: router.get('/scene-status/:taskId/:sceneTaskId', ...).
        // The first param is PARENT taskId.
        // We need to pass that.
        // Let's assume the caller passes taskId as first arg, not generationId.
        // But signature says generationId.
        // Let's update signature to taskId.
        // For now, let's keep it as is but construct URL assuming first arg is taskId.
        const response = await api.get<{
            sceneTaskId: string,
            status: string,
            ...
            sceneUrls: string[]
        }>(`/video/scene-status/${generationId}/${sceneTaskId}`) // Assuming first arg is actually taskId
        return response.data
    },
    ...
    getCombineStatus: async (requestId: string, generationId: string) => {
        // Server route: /combine-status/:requestId
        // But query param taskId (or generationId fallback in code?)
        // Server code: const { taskId } = req.query
        // So we pass taskId or generationId in params.
        const response = await api.get<{
            requestId: string,
            status: string,
            completed: boolean,
        ...
        }>(`/video/combine-status/${requestId}`, { params: { generationId } }) // Using generationId as context if needed
        return response.data
    },

    // Start lip sync for a scene
...
// Unified Videos CRUD API (replaces productsApi and videosLibraryApi)
export const videosApi = {
    ...
    // Get only completed videos (for library)
    getCompleted: async () => {
...
// Backward compatibility aliases (deprecated - use videosApi instead)
export const productsApi = {
    save: async (imageUrl: string, name?: string) => {
        // Create a video record with product info
...
export const videosLibraryApi = {
    save: async (videoUrl: string, thumbnailUrl?: string, title?: string, _duration?: number) => {
        // This is now handled by updating the existing video record
        // But for backward compatibility, we create a new record
